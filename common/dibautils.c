...

{
    ...
    if(pks->thispass[0] == '\0' && !pks->nocrypt)
        {
        getpassx  passx; ZERO_GETP_STRUCT(&passx);
...

{
    ...
    char hash_two[32];
    gcry_md_hash_buffer(GCRY_MD_SHA256, (void*) &hash_two, 
                    (const void*) data_buf, data_len);
...
void show_prepoc()
{
        ...
        printf("defined __WINNT__\n");
    #endif    
    #if defined WINNT 
        printf("defined WINNT\n");
    #endif    
